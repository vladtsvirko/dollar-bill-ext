const RatesUtil = (() => {
  ...
  const RATE_SOURCES = {
    ...
    boc: {
      ...
      convention: 'direct',
      fetchBaseRates: async () => {
        const resp = await fetch('https://www.bankofcanada.ca/valet/fx_daily.json');
    ...
    bcb: {
      ...
      fetchBaseRates: async () => {
        ...
        if (data.value && data.value.length > 0) {
          ...
          // USD/BRL rate from cotacaoCompra (buying rate)
    ...
    boe: {
      ...
      convention: 'direct',
      fetchBaseRates: async () => {
        const resp = await fetch('https://www.bankofengland.co.uk/boeapps/database/_iadb-fromshow.asp?StatTypes=CSV&SeriesCodes=XUDLUSS,XUDLERD,XUDLJYS,XUDLCHF,XUDLCAD,XUDLAUD&xml.x=1&CSVf=TN&UsingCodes=Y&VPD=Y&VFD=N');
        if (!resp.ok) throw new Error(`BOE API error: ${resp.status}`);
        const text = await resp.text();
        const rates = { GBP: 1 };
        // Series code -> currency mapping
        const seriesMap = {
          XUDLUSS: 'USD',
          XUDLERD: 'EUR',
          XUDLJYS: 'JPY',
        ...
        // Parse CSV: find header line, then last data line
        const lines = text.split('\n').filter(l => l.trim());
        // Find the header line containing series codes
        let headerIdx = -1;
        for (let i = 0; i < lines.length; i++) {
          if (lines[i].includes('XUDL')) {
            headerIdx = i;
            break;
          }
        }
        if (headerIdx < 0) return { base: 'GBP', rates };
        const headers = lines[headerIdx].split(',');
        const seriesIndices = {};
        for (const [code, currency] of Object.entries(seriesMap)) {
          const idx = headers.indexOf(code);
          if (idx >= 0) seriesIndices[code] = idx;
        }
        // Use last data line
        const lastLine = lines[lines.length - 1];
        const values = lastLine.split(',');
        for (const [code, idx] of Object.entries(seriesIndices)) {
          const val = parseFloat(values[idx]);
          if (!isNaN(val) && val > 0) {
            rates[seriesMap[code]] = val;
          }
        }
